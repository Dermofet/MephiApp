...
from backend.Schemas.Group import Group, GroupCreate, GroupOutput
...
class GroupService:
    ...
    async def create(db: AsyncSession, schemas: GroupCreate) -> GroupOutput:
        ...
        return GroupOutput(**Group.from_orm(group).dict())
    ...
    async def get_all(db: AsyncSession) -> list[str]:
        ...
        if groups is None:
            raise HTTPException(404, "Группа не найдена")
        return [Group.from_orm(group).name for group in groups]
