...
class GroupRepository:
    @staticmethod
    async def create(db: AsyncSession, schemas: GroupCreate, academic_guid) -> Group:
        group = Group(**schemas.dict(exclude_unset=True), academic_guid=academic_guid)
        db.add(group)
        await db.commit()
    ...
    async def get_by_id(db: AsyncSession, guid: UUID4) -> Group:
        ...
        if len(group.scalars().all()) > 0:
            return group.scalars().all()[0]
        return None
